"""MobileOne-S0 baseline training script matching LiteJam's multi-task setup."""
...
from baseline.mobileone import MobileOneS0Baseline, MobileOneS0BaselineConfig
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train MobileOne-S0 baseline for LiteJam tasks")
    ...
    parser.add_argument("--freeze-backbone", action="store_true", help="Freeze MobileOne backbone weights")
    parser.add_argument(
        "--channel-strategy",
...
def save_checkpoint(
    model: MobileOneS0Baseline,
    model_cfg: MobileOneS0BaselineConfig,
    ...
    if suffix in {"best", "good", "last"}:
        ckpt_path = save_dir / f"mobileone_{run_stamp}_{suffix}.pt"
    else:
        stamp = datetime.now().strftime("%Y%m%d-%H%M%S-%f")
        ckpt_path = save_dir / f"mobileone_{run_stamp}_{stamp}_{suffix}.pt"
    torch.save(
        {
...
def main() -> None:
    ...
    if args.pretrained and in_channels != 3:
        raise ValueError("Pretrained MobileOne-S0 requires 3 input channels; choose a different channel strategy")
    ...
    model = MobileOneS0Baseline(model_cfg).to(device)
