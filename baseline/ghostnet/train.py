"""GhostNet-1.0 baseline training script matching LiteJam's multi-task setup."""
...
from baseline.ghostnet import GhostNetBaseline, GhostNetBaselineConfig
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train GhostNet baseline for LiteJam tasks")
    ...
    parser.add_argument("--freeze-backbone", action="store_true", help="Freeze GhostNet backbone weights")
    parser.add_argument(
        "--channel-strategy",
...
def save_checkpoint(
    model: GhostNetBaseline,
    model_cfg: GhostNetBaselineConfig,
    ...
    if suffix in {"best", "good", "last"}:
        ckpt_path = save_dir / f"ghostnet_{run_stamp}_{suffix}.pt"
    else:
        stamp = datetime.now().strftime("%Y%m%d-%H%M%S-%f")
        ckpt_path = save_dir / f"ghostnet_{run_stamp}_{stamp}_{suffix}.pt"
    torch.save(
        {
...
def main() -> None:
    ...
    if args.pretrained and in_channels != 3:
        raise ValueError("Pretrained GhostNet requires 3 input channels; choose a different channel strategy")
    ...
    model = GhostNetBaseline(model_cfg).to(device)
