"""EfficientFormer-L1 baseline training script matching LiteJam's multi-task setup."""
...
from baseline.efficientformer import EfficientFormerL1Baseline, EfficientFormerL1BaselineConfig
try:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train EfficientFormer-L1 baseline for LiteJam tasks")
    ...
    parser.add_argument("--freeze-backbone", action="store_true", help="Freeze EfficientFormer backbone weights")
    parser.add_argument(
        "--channel-strategy",
...
def save_checkpoint(
    model: EfficientFormerL1Baseline,
    model_cfg: EfficientFormerL1BaselineConfig,
    ...
    if suffix in {"best", "good", "last"}:
        ckpt_path = save_dir / f"efficientformer_{run_stamp}_{suffix}.pt"
    else:
        stamp = datetime.now().strftime("%Y%m%d-%H%M%S-%f")
        ckpt_path = save_dir / f"efficientformer_{run_stamp}_{stamp}_{suffix}.pt"
    torch.save(
        {
...
def main() -> None:
    ...
    spatial_size = tuple(int(dim) for dim in sample_tensor.shape[-2:])
    if args.pretrained and in_channels != 3:
        raise ValueError("Pretrained EfficientFormer-L1 requires 3 input channels; choose a different channel strategy")
    ...
    model = EfficientFormerL1Baseline(model_cfg).to(device)
