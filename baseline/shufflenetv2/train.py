"""ShuffleNetV2-1.0x baseline training script matching LiteJam's multi-task setup."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train ShuffleNetV2 baseline for LiteJam tasks")
    ...
    parser.add_argument("--freeze-backbone", action="store_true", help="Freeze ShuffleNet backbone weights")
    parser.add_argument(
        "--channel-strategy",
...
def save_checkpoint(
    ...
    model_cfg: ShuffleNetV2BaselineConfig,
    ...
    if suffix in {"best", "good", "last"}:
        ckpt_path = save_dir / f"shufflenetv2_{run_stamp}_{suffix}.pt"
    else:
        stamp = datetime.now().strftime("%Y%m%d-%H%M%S-%f")
        ckpt_path = save_dir / f"shufflenetv2_{run_stamp}_{stamp}_{suffix}.pt"
    torch.save(
        {
...
def main() -> None:
    ...
    if args.pretrained and in_channels != 3:
        raise ValueError("Pretrained ShuffleNetV2 requires 3 input channels; choose a different channel strategy")
    ...
    model = ShuffleNetV2Baseline(model_cfg).to(device)
