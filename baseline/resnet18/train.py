"""ResNet18 baseline training script matching LiteJam's multi-task setup."""
...
from baseline.resnet18 import ResNet18Baseline, ResNet18BaselineConfig
try:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train ResNet18 baseline for LiteJam tasks")
...
def save_checkpoint(
    ...
    model_cfg: ResNet18BaselineConfig,
    ...
    if suffix in {"best", "good", "last"}:
        ckpt_path = save_dir / f"resnet18_{run_stamp}_{suffix}.pt"
    else:
        stamp = datetime.now().strftime("%Y%m%d-%H%M%S-%f")
        ckpt_path = save_dir / f"resnet18_{run_stamp}_{stamp}_{suffix}.pt"
    torch.save(
        {
...
def main() -> None:
    ...
    if args.pretrained and in_channels != 3:
        raise ValueError("Pretrained ResNet18 requires 3 input channels; choose a different channel strategy")
    ...
    model = ResNet18Baseline(model_cfg).to(device)
